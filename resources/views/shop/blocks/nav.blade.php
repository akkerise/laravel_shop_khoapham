<div id="categorymenu">
  <nav class="subnav">
    <ul class="nav-pills categorymenu">
      ...
      ?>
      <li>
        <a href="{{ secure_url('/') }}">Trang Chủ</a>
      </li>
      @foreach($cate_level_1 as $item_level_1)
      <li>
        <a href="{{ route('listProducts',[$item_level_1->id,$item_level_1->name]) }}">{{ $item_level_1->name }}</a>
        <div>
          <ul>
            <?php
            ...
            <li>
              <a href="{{ route('listProductsDetail',$item_level_1->id) }}">{{ $item_level_2->name }}</a>
            </li>
            @endforeach
          </ul>
        </div>
